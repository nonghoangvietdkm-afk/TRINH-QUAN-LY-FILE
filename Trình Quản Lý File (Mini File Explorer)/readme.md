...
Để chạy file kiểm thử tự động (5 test cases):
g++ -std=c++17 tests/test.cpp src/functions.cpp -o test_app && ./test_app

## Chức năng
1. Hiển thị cây thư mục dưới dạng đồ họa ASCII trực quan (giống lệnh tree).
2. Tạo mới (mkdir, touch), xóa bỏ (rm), và đổi tên (rename) cho cả folder và file.
3. Di chuyển và quản lý vị trí hiện tại trong hệ thống thư mục (cd, ls, pwd).
4. Di chuyển lùi lại (Back) hoặc tiến lên (Forward) trong lịch sử bằng Stack.
5. Tìm kiếm tập tin hoặc thư mục theo tên bằng thuật toán duyệt theo chiều rộng BFS (Queue).
6. Tính toán và hiển thị tổng dung lượng của một thư mục bằng thuật toán duyệt cây Post-order traversal.
7. Lưu toàn bộ cấu trúc cây thư mục ra file .txt và đọc/tái tạo lại cây từ file đó (I/O Serialization).
...
tests/
    test.cpp       — Chạy tự động 5 test case chính